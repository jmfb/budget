...
public class HomeController : Controller {
	...
	public async Task<IActionResult> Index(
		...
		var firstKey = DataVersionNames.TransactionsByWeek(startOfXWeeksAgo.ToString("yyyy-MM-dd"));
		var lastKey = DataVersionNames.TransactionsByWeek(startOfWeek.ToString("yyyy-MM-dd"));
		...
		var weekVersionsTask = DataVersionsService.GetVersionsAsync(firstKey, lastKey, cancellationToken);
		...
		var weekVersions = Enumerable
			.Range(0, weeksAgo + 1)
			.Select(index => startOfXWeeksAgo.AddDays(7 * index).ToString("yyyy-MM-dd"))
			.ToDictionary(
				weekOf => weekOf,
				weekOf => weekDataVersions
					.TryGetValue(
