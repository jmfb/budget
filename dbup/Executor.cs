...
public class Executor(DatabaseOptions options)
{
	...
	public async Task<int> ApplyAsync() =>
		await RunAllDeploymentsAsync() ? 0 : 1;
	...
	private async Task<bool> RunMasterDeploymentAsync()
	{
		...
		return RunDeployment(
			MasterDeployment,
			"Running pre-deployment script against master database"
		);
	}
	...
	private bool RunPreDeployment() =>
		RunDeployment(
			PreDeployment,
			"Running pre-deployment script against service database"
		);
	...
	private bool RunStoredProcedureDeployment() =>
		RunDeployment(
			StoredProcedureDeployment,
			"Running stored procedure deployment"
		);
	...
	private UpgradeEngine MasterDeployment =>
		DeployChanges
			...
			.WithScriptsEmbeddedInAssembly(
				...
				scriptName =>
					scriptName.Contains("DbUp.Scripts.PreDeployment.Master")
			)
			.LogToConsole()
			.Build();

	private UpgradeEngine PreDeployment =>
		DeployChanges
			...
			.WithScriptsEmbeddedInAssembly(
				Assembly.GetExecutingAssembly(),
				scriptName =>
					scriptName.Contains("DbUp.Scripts.PreDeployment.Service")
			)
			.LogToConsole()
			.Build();

	private UpgradeEngine SchemaDeployment =>
		DeployChanges
			.To.PostgresqlDatabase(options.ServiceConnectionString)
			.WithVariables(
				new Dictionary<string, string>
	...
	private UpgradeEngine StoredProcedureDeployment =>
		DeployChanges
			.To.PostgresqlDatabase(options.ServiceConnectionString)
			.JournalTo(new NullJournal())
			.WithScriptsEmbeddedInAssembly(
				Assembly.GetExecutingAssembly(),
				scriptName =>
					scriptName.Contains("DbUp.Scripts.StoredProcedures")
			)
			.LogToConsole()
			.Build();
}
